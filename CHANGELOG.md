...
- **Claude 设置 · 命令与技能**：拆分为 **自定义命令** 与 **技能** 两组配置；各预览至多 **3** 条；「查看全部」与弹窗 **按类型独立计数与标题**；删除前 **确认对话框**；删除后与刷新路径调用 **`catalog.refresh()`** 并通知清空对话斜杠缓存；详见 **`docs/01-Projects/R20260429-12-分离技能和命令/`**。
